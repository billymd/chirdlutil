...
public class VoiceSystemUtil {
	...
	public static String uploadCallData(List<VoiceCallRequest> callRequests) throws IOException {
		...
		for (VoiceCallRequest callRequest : callRequests) {
			Patient patient = callRequest.getPatient();
			...
			if (phoneNumber == null || phoneNumber.trim().length() == 0) {
				...
				log.error("Patient " + patientId + " does not have a phone number and will not be forwarded to the " +
			...
			if (locationName == null) {
				...
				if (locationName == null || locationName.trim().length() == 0) {
					...
					log.error("Location " + locationId + " does not have a displayName location attribute defined.  Patient " +
							patientId + " will not be referred to the voice system service to be called.");
					continue;
				}
		...
		// Perform the POST
		String result = HttpUtil.post(url, data, connectionTimeout, readTimeout);
		return result;
	}
	
	/**
	 * Utility method to get a patient's phone number.  This may return null if the patient does not have a phone number.
	 * 
	 * @param patient The patient used to retrieve the phone number.
	 * @return Phone number of the provided patient or null if one cannot be found.
	 */
	private static String getPatientPhoneNumber(Patient patient) {
		PersonAttribute patientAttr = patient.getAttribute("Telephone Number");
		if (patientAttr == null) {
			return null;
		}
		
		return patientAttr.getValue();
	}
